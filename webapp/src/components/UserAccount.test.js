...
describe('UserAccount Component', () => {
  ...
  test('fetches and displays user statistics', async () => {
    ...
    expect(await screen.findByText('Preguntas acertadas: 14')).toBeInTheDocument();
    expect(await screen.findByText('Preguntas falladas: 6')).toBeInTheDocument();
    expect(await screen.findByText('Total de preguntas: 20')).toBeInTheDocument();
  });
